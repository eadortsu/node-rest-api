...
handlers._eits.post = (data, callback) => {
    ...
    if (!Object.keys(errors).length) {
        ...
        _data.read('eits', email, (err, data) => {
            if (err) {
                ...
                // Store the user
                _data.create('eits', email, eitObject, (err) => {
                    if (!err) {
                ...
                // Eit already exists
                callback(400, {'Error': 'An eit with that Email already exists'})
            }
        })
...
handlers._eits.get = (data, callback) => {
    ...
    //Verify that the givent token if valid for the username
    const email = typeof (data.queryStringObject.email) == 'string' ? data.queryStringObject.email.trim() : false
...
handlers._eits.put = (data, callback) => {
    ...
    if (!email) {
        ...
        if (firstName || lastName || age || country) {
            // Lookup the user
            _data.read('eits', email, (err, eitData) => {
                if (!err && eitData) {
                    ...
                    // Store the new updates
                    _data.update('eits', email, eitData, (err) => {
                        if (!err) {
