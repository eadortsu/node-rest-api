    ...
    if (!token) {
        ...
        handlers._tokens.getCurrentUser(token, (currentUser) => {
            if (!currentUser) {
                ...
                if (!Object.keys(errors).length) {
                    // Make sure the user doesnt already exist
                    _data.read('eits', firstName + '.' + lastName, (err, data) => {
                        if (err) {
                            ...
                            // Store the user
                            _data.create('eits', firstName + '.' + lastName, eitObject, (err) => {
                                if (!err) {
    ...
    if (!token) {
        ...
        handlers._tokens.getCurrentUser(token, (currentUser) => {
            if (!currentUser) {
                ...
                // Read all eits
                _data.readAll('eits', (data, err) => {
                    if (!err) {
                        const allEits = {}
                        data.forEach((eit) => {
                            _data.read('eits', eit, (err, data) => {
                                if (!err && data) {
                                    // Remove the hashed password from the user user object before returning it to the requester
        ...
        handlers._tokens.getCurrentUser(token, (currentUser) => {
            if (!currentUser) {
                ...
                if (firstName || lastName || age || country) {
                    // Lookup the user
                    _data.read('eits', firstName+'.'+lastName, (err, eitData) => {
                        if (!err && eitData) {
                            // Update the fields if necessary
                            if (firstName) {
                            ...
                            if (age) {
                                eitData.hashedPassword = age
                            }
                            if (country) {
                                eitData.hashedPassword = country
                            }
                            // Store the new updates
                            _data.update('eits', firstName+'.'+lastName, eitData, (err) => {
                                if (!err) {
...
handlers._eits.delete = (data, callback) => {
    ...
    if (username) {
        ...
        handlers._tokens.verifyToken(token, username, (tokenIsValid) => {
            if (tokenIsValid) {
                // Lookup the user
                _data.read('eits', username, (err, data) => {
                    if (!err && data) {
                        _data.delete('eits', username, (err) => {
                            if (!err) {
                                callback(200, {'Msg': 'User Deleted'})
                            } else {
                                callback(500, {'Error': 'Could not delete the specified user'})
